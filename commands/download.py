...
async def handle_url_message(client: Client, message: Message):
    ...
    try:
        ...
        # Start download in background (always best quality)
...
async def download_and_send_concurrent(client, message, progress_tracker, user_id):
    ...
    try:
        ...
        # Configure yt-dlp options (always best quality)
        try:
            ydl_opts = get_download_options(url)
            ydl_opts.update({
                'format': 'best',  # Always download best quality
        ...
        try:
            print(f"Starting yt-dlp download for URL: {url}")
